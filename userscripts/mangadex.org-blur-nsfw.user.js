...
(function () {
	...
	function addHoverEffect(parent, img, blurLevel, unblurLength, positionElement) {
		...
		let cover = positionElement || parent.getElementsByClassName('manga-card-cover')[0];
	...
	function processMangaCard(mangaCard) {
		...
		for (let tagElement of tagElements) {
			...
			if (isNsfwTag(tagText)) {
				...
				let img = cover.getElementsByTagName('a')[0].getElementsByTagName('img')[0];
	...
	function processMangaSwiperSlide(mangaSwiperSlide) {
		...
		let tagsDiv = div.querySelector("div[class*='flex'][class*='flex-wrap'][class*='gap-1']");
		if (!tagsDiv) return;
	...
	function isValidPage() {
		const validPaths = [
			/^\/$/, // home page
			/^\/titles(\/.*)?$/, // titles and title details pages
			/^\/list\/.*$/, // list pages
			/^\/author\/.*$/ // author pages
		];

		return validPaths.some((regex) => regex.test(window.location.pathname));
	}
